...
class InstrumentProcessor:
    ...
    def get_instrument_data(self, k_line_type):
        ...
        # 构建数据路径
        data_path = f'{DATA_DIR}/{k_line_type}/{self.instrument}/{self.instrument}.csv'
    ...
    def calculate_factor_for_kline(self, k_line_type):
        ...
        for factor_params in factor_params_list:
            ...
            # 设置因子保存路径
            save_path = f'{DATA_DIR}/{k_line_type}/{self.instrument}/{full_name}.csv'
...
def run_instrument_processor(args):
    """包装函数用于进程池"""
    instrument, k_line_type, instruments_mindiff, factor_json_path = args
    processor = InstrumentProcessor(instrument, k_line_type, instruments_mindiff, factor_json_path)
