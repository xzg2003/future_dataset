...
|           因子名称           |                             说明                             | 是否完成计算 | 是否完成评估 | 是否完成核对 |
| :--------------------------: | :----------------------------------------------------------: | :----------: | :----------: | :----------: |
|         FCT_Ac_Tr_1          |                  技术指标AC除以MA(TR)去量纲                  |      是      |      是      |              |
|           FCT_Ar_1           |          人气意愿指标，high、open、low差值累加对比           |      是      |      是      |              |
|          FCT_Bias_1          |                      乖离率除以Tr去量纲                      |      是      |      是      |              |
|           FCT_Br_1           |       人气意愿指标high, ref(close, 1), low差值累加对比       |      是      |      是      |              |
|          FCT_Cmf_1           | 资金流量指标，若收盘价在上半部分，且成交量放大，表示做多积极 |      是      |      是      |              |
|          FCT_Pubu_1          | 普及率因子，由短期均线和长期均线两条均线组成，衡量短期均线在长期均线窗口内的分位数位置 |      是      |      是      |              |
|      FCT_Pubu_Atr_Dfive      | 衡量短期均线在长期均线窗口内的分位数位置，并用 ATR 归一化，适合捕捉价格分布于波动率的关系 |      是      |      是      |              |
|      FCT_Pubu_Vol_Dfive      | 衡量短期均线在长期均线窗口内的分位数位置，并用成交量归一化，适合捕捉价格分布与成交活跃度的关系 |      是      |      是      |              |
|        FCT_R_Div_RStd        | 收益率与波动率比值因子，衡量单位波动下的收益率（动量/风险调整） |      是      |      是      |              |
|     FCT_Return_Cumsum_1      |        累积收益率因子，衡量从起始到当前的累积对数收益        |      是      |      是      |              |
|          FCT_Rsi_1           |                         相对强弱指标                         |      是      |      是      |              |
|          FCT_Sdrm_1          |        标准动量差因子，衡量收盘价在指定窗口内的波动性        |      是      |      是      |              |
|      FCT_Sdrm_Atr_Dfive      |  标准差动量与 ATR 归一化因子，衡量价格波动性与波动率的关系   |      是      |      是      |              |
|            FCT_Si            |     SI（波动率指标）因子，衡量收盘价的绝对变化幅度的均值     |      是      |      是      |              |
|           FCT_Srmi           | SRMI（相对动量指数）因子，衡量收盘价在指定窗口内的相对强弱位置 |      是      |      是      |              |
|   FCT_Support_Close_Thr_1    | 收盘价接近支撑位的次数因子，衡量收盘价在窗口内低于一定分位数的次数 |      是      |      是      |              |
| FCT_Support_Close_Thr_Boll_1 | 收盘价接近布林带下轨的次数因子，衡量收盘价在窗口内低于下轨的次数 |      是      |      是      |              |
|          FCT_Tsi_1           |                   TSI（趋势强度指标）因子                    |      是      |      是      |              |
|      FCT_Tsi_Atr_Dfive       |     趋势强度与 ATR 归一化因子，衡量单位波动下的趋势强度      |      是      |      是      |              |
|        FCT_TSI_Ref_1         |    TSI（真实强度指标）参考因子，衡量价格趋势的强弱和方向     |      是      |      是      |              |
|      FCT_Tsi_Vol_Dfive       |    趋势强度与成交量归一化因子，衡量单位成交量下的趋势强度    |      是      |      是      |              |
|          FCT_Vmacd           |       成交量 MACD 因子，衡量成交量的趋势变化和动能拐点       |      是      |      是      |              |
|     FCT_Vol_Close_Corr_1     | 成交量与收盘价相关系数因子，衡量指定窗口内成交量与收盘价的相关性 |      是      |      是      |              |
|       FCT_Vol_Cumsum_1       |         成交量累计因子，衡量从起始到当前的累计成交量         |      是      |      是      |              |
|       FCT_Vol_DFive_1        | 成交量动量因子，衡量成交量在指定窗口内的变化幅度（如标准差或均值） |      是      |      是      |              |
|    FCT_Vol_Return_Corr_1     | 成交量与收益率相关系数因子，衡量指定窗口内成交量与收益率的相关性 |      是      |      是      |              |
|            FCT_Vr            |    量比因子，衡量当前成交量与过去一段时间平均成交量的比值    |      是      |      是      |              |
