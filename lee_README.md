...
- **因子名称处理方式：**结合上述对因子命名逻辑的修改，在程序中可以对因子名称的处理方式进行修改。原有的处理方式是用一大段的判断语句对每个因子进行分类，再进行分类命名。而且因子计算器还对这个部分进行了多重循环，导致整个程序的可读性较差。对因子结果命名进行修改后，一个因子的计算结果可以分为两类：一类不带有“@”这个特殊字符，意味着这个因子包含了对长度的处理；另一类就是带有“@”这个字符的类型，前面是因子的基础名称，后面是因子的长度类型，将这个名称前后分割，就变成 {factor_name} 和 {length} 部分，对于 {length} 部分，我们可以通过数据类型转换，将其转换为整形再变成参数进行计算，这样可以省去对 length 的循环，增加可读性，也可省去相当一部分不必要的循环。在第一次尝试中，暂时没有对 length 进行省略，避免大规模的变动导致程序的崩溃。

#### 2025-06-13	**Task_3_1.1.5**

- 对 factor_calculator 的参数导入规则进行了修改，删除了先前需要的 lengths 这个参数的导入。因为现在对 length 的导入是利用因子名称里自带的长度进行导入，不需要在外部再额外地导入参数。
- 对 delete.py 进行同样的修改，利用 factor_name 实现对因子名称的导入，并删除相应的因子计算结果。同时还对 delete.py 进行优化，允许用户进行模糊匹配以删除因子计算结果。
